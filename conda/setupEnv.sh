...
function run_setup()
{
    ...
    if [ $(uname) == "Darwin" ]; then
        ...
        curl -sS https://repo.anaconda.com/miniconda/Miniconda3-py311_23.5.2-0-MacOSX-x86_64.sh -o $INSTALL_ABSDIR/conda.sh || return 1
    elif [ "$(expr substr $(uname -s) 1 5)" == "Linux" ]; then
        echo "Installing for Linux"
        curl -sS https://repo.anaconda.com/miniconda/Miniconda3-py311_23.5.2-0-Linux-x86_64.sh -o $INSTALL_ABSDIR/conda.sh || return 1
    fi
    ...
    echo "source activate tagger" >> $SCRIPT_DIR/env.sh
